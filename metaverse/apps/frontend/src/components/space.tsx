...
const PhaserMultiplayer = () => {
      ...
      if (typeof window !== 'undefined') {
        ...
        class MainScene extends Phaser.Scene {
          ...
          preload() {
            ...
            this.load.image("castleimage", localimage);
          }

          create() {
            const mapWidth = 1000, mapHeight = 1000, tileSize = 64;
            const graphics = this.add.graphics();
            graphics.lineStyle(1, 0xcccccc);
            this.add.image(0, 0, "castleimage").setOrigin(0, 0).setScale(1);
          ...
          initializePlayers() {
            const localPlayer = this.createPlayer('self', 400, 300, 0x3b82f6, 'You');
            ...
            otherPlayers.forEach(player => {
              const otherPlayer = this.createPlayer(
                player.id, player.x, player.y, player.color, player.name
              );
          ...
          createPlayer(id, x, y, color, name) {
            ...
            player.playerId = id;
            const playerText = this.add.text(x, y - 35, name, {
            ...
            this.players.forEach((otherPlayer, id) => {
              if (id !== 'self') {
                const distance = Phaser.Math.Distance.Between(
                  localPlayer.x, localPlayer.y, otherPlayer.x, otherPlayer.y
                );
                if (distance < INTERACTION_DISTANCE) {
                  otherPlayer.interactionCircle.setVisible(true);
                  otherPlayer.interactionCircle.setAlpha(1 - (distance / INTERACTION_DISTANCE));
                } else {
          ...
          update() {
            ...
            if (velocityX !== 0 || velocityY !== 0) {
              console.log('Player moved:', { x: localPlayer.x, y: localPlayer.y });
            }
          }
        }
  ...
  return (
    <div className="relative w-full h-screen">
      <div id="phaser-container" className="absolute inset-0 max-width-full max-height-full" style={{ backgroundColor: '#e5e7eb' }} />
      <div className="absolute top-4 left-4 bg-white p-6 rounded-lg shadow-lg border border-gray-200">
        ...
        <ul className="space-y-2">
          {Object.values(players).map(player => (
            <li key={player.id} className="flex items-center gap-2 p-2 rounded hover:bg-gray-100 transition duration-200">
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: player.id === 'self' ? '#3b82f6' : player.id === 'user1' ? '#10b981' : '#f59e0b' }} />
              {player.name}
            </li>
          ))}
        </ul>
